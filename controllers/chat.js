...
const newGroupChat = TryCatch( async(req,res,next) => {
    ...
    emitEvent(req, ALERT, allMembers, `Welcome to ${name} groups`);
    emitEvent(req, REFETCH_CHATS, members);

    return res.status(201).json({
        success: true,
        message: "Group Cheated",
...
const getMyChats = TryCatch( async(req,res,next) => {
    ...
    const transformedCats = chats.map(({_id,name,members,groupChat}) => {
    ...
    const sendAttachments = TryCatch(async (req, res, next) => {
        ...
        if(files.length > 10)
            return next(new ErrorHandler("File Can't be more then 10",400));
